...
This is an step by step method of mining the association rules from transaction data. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents.The pure source code is available in .R file inside the repository <http://rmarkdown.rstudio.com>.
...
offset <-0
```
We are done with the reading of file. We will now run the apriori for each of the city
```{r eval=FALSE}
